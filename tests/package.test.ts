  ...
  writeFileSync(
    ...
    void context?.registerTool({
      ...
      execute(input) {
        ...
        const labels: string[] | undefined = input.options?.labels;
        // @ts-expect-error The schema infers count as a number.
        const invalid: string = input.count;
        // @ts-expect-error Nested labels are strings, not numbers.
        const invalidLabels: number[] | undefined = input.options?.labels;
        return { count, labels };
      },
    });
